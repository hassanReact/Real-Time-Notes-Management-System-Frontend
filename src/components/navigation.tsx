...
import { CollaborationIndicators } from "./collaborative-indicator"
import { RealTimeNotifications } from "./real-time-notification"
import { useAuth } from "@/hooks/use-auth"
import { getUserInitials, getUserDisplayName, getUserEmail } from "@/lib/avatar-utils"
...
export function Navigation({ children }: NavigationProps) {
  ...
  // Search results will come from backend API
  const searchResults: Array<{ id: string; title: string; type: string }> = []
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="flex">
        ...
        <aside className="fixed left-0 top-16 h-[calc(100vh-4rem)] w-64 border-r border-border bg-muted/20 hidden md:block">
          <div className="flex h-full flex-col">
            {/* No overflow classes here => fixed and not scrollable */}
        ...
        {/* Main Content: shift right on md+ so it doesn't sit under the fixed sidebar */}
          ...
          <CommandGroup heading="Recent Notes">
            {searchResults.length > 0 ? (
              ...
              <CommandItem disabled>
                <span className="text-muted-foreground">No recent notes found</span>
              </CommandItem>
            )}
          </CommandGroup>
        </CommandList>
